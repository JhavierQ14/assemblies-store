...
document.addEventListener('DOMContentLoaded', function() {
  ...
  otpInputs.forEach((input, idx) => {
    ...
    input.addEventListener('input', function(e) {
      ...
      if (val.length > 1) {
        ...
        for (let i = 0; i < digits.length; i++) { const target = otpInputs[idx + i]; if (!target) break; target.value = digits[i]; }
    ...
    input.addEventListener('keydown', function(e) {
      const key = e.key;
      if (key === 'Backspace') { if (!input.value && idx > 0) { e.preventDefault(); otpInputs[idx - 1].focus(); otpInputs[idx - 1].value = ''; } return; }
  ...
  confirmResendBtn && confirmResendBtn.addEventListener('click', async function(e) {
    ...
    try {
      ...
      resendModal.classList.add('hidden'); try { emailInput.value = email; otpInputs.forEach(i => i.value = ''); if (otpInputs.length) otpInputs[0].focus(); } catch (e) {}
      markOtpSent(email);
